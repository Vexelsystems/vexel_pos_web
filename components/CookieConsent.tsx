...
export default function CookieConsent() {
  const [status, setStatus] = useState<
    "loading" | "unset" | "accepted" | "rejected"
  >("loading");

  /**
   * CONSENT STATE MACHINE
   * Strategy: localStorage sync with React state.
   * Logic:
   * 1. 'loading': Initial state to prevent UI flicker before hydration.
   * 2. 'unset': User hasn't decided (renders the banner).
   * 3. 'accepted': User agreed (renders nothing).
   * 4. 'rejected': User declined (renders the access wall).
   */
  useEffect(() => {
  ...
  return (
    <>
      ...
      {status === "rejected" && (
        <div className="fixed inset-0 z-10000 bg-white/80 dark:bg-black/80 backdrop-blur-2xl flex items-center justify-center p-6">
          <div className="max-w-md text-center">
            ...
            <p className="text-foreground/60 mb-8 leading-relaxed">
              To browse Vexel Systems, you must accept our cookie policy. We use
              these to ensure site security and optimal performance.
            </p>
      ...
      {/* The Initial Banner */}
      {status === "unset" && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-999 w-[90%] max-w-[700px]">
          <div className="bg-white dark:bg-zinc-900 border border-black/5 dark:border-white/10 rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 backdrop-blur-xl">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-sm text-foreground/60 leading-relaxed">
                ...
                to access all features of Vexel Systems.
              </p>
            </div>
